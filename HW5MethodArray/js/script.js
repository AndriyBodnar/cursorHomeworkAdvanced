// 1
  ...
  const getMedian = (...numbers) => {
    ...
    let middlePos = Math.round(sortNum.length / 2);
    if (sortNum.length % 2 === 0) {
      return sortNum[Math.floor(middlePos)];
    } else {
      return (sortNum[middlePos] + sortNum[middlePos - 1]) / 2;
    }
  };
  const median = getMedian(1, 112, 2, 22, 4, 33, 3, 4.4)
  console.log(median);
  ...
  const filterEvenNumbers = (...numbers) => {
    return numbers
      .filter((number) => number % 1 === 0 && number % 2)
  };
  ...
  const fiveDiv = getDividedByFive(6, 2, 55, 11, 78, 2, 575, 77, 57, 87, 23, 2, 56, 3, 2, 105, 105.55)
  console.log(fiveDiv);
  ...
  div.innerHTML = `
   ...
   <p><b>Func #8 (bad Words):</b></br>${replaceBadWords("Are you fucking kidding? It's bullshit!")}</p>
   <p><b>Func #9 (divideByThree):</b></br>${divideByThree("Авіаконструктор")}</p>
